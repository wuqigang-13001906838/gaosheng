$(document).ready(function(){
	...
	var firstA = $(".banner li").first().clone();
	/*复制最后一张图片  .clone() jquery中用于复制元素的方法*/
	var lastA = $(".banner li").last().clone();
	/*将第一张复制的元素插入到最后一个元素的后面*/
	$(".banner li").last().after($(firstA));
	/*将最后一张复制的元素插入到第一个元素的前面*/
	$(".banner li").first().before($(lastA));
	/*轮播图的数量*/
	var aNum = $(".banner li").length;
	/*单张图片的宽度*/
	var aWidth = $(".banner li").width();
	/*设置图片外层盒子的宽度*/
